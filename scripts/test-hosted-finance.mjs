...
test('Wallet imports are host-only, refresh mappings, validate before writes and retain retry identity',async()=>{
 ...
 const broken=structuredClone(parts);broken[0].transactions[0].amount='NaN';await assert.rejects(()=>h.backend.importWallet(broken));assert.equal(writes,2);assert.equal(prepared,2,'invalid batches never create folders');
