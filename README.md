## 1.3.0 — Plaid through Controller

Plaid setup now hands off to TPS Controller → Advanced → Plaid. The former environment, client ID, secret, and redirect controls live there as device-specific settings. Controller 1.3.0 or later owns authenticated HTTP requests; Finances has no direct Plaid transport fallback. Missing Controller pauses provider operations with an actionable error. Connections, account management, sync, manual cash/assets, routing, rules, and budgets remain in Finances. Existing SecretStorage access tokens, identities, and cursors stay intact; cursors still commit only after ledger writes. Install Controller first. No bank reconnection or note migration is required.

Verified in the reloaded test vault: Setup opens Controller’s Plaid settings; Connections retains Connect, Sync, Reconnect, and Disconnect and the existing Sandbox institution records. Mock-provider tests cover transport delegation, missing Controller, credential-reference preservation through Link, pagination, ledger/cursor ordering, and existing manual cash/assets. No live bank/ Sandbox API requests were made. Desktop-only authentication remains the existing limitation. Full declared npm test and a separate final npm run build are required for the versioned artifacts; the stable build deploys only shipped files to Obsidian Plugin Test Vault. Hot Reload plus explicit test-plugin reload was used for UI verification. Release notes record final validation and SHA-256 hashes. Production installation remains the user’s BRAT pull.

# 1.2.0 — Manual cash and resale assets

**Add → Cash account / Cash transaction / Resale asset** supports finances without a Plaid connection. The corresponding commands are **Add cash account**, **Log cash transaction**, and **Add resale asset**. All five previous commands and the Rule, Budget, Connect, and Sync actions remain. Settings retain their four destinations and existing defaults; this feature adds no persisted settings or navigation state.

Cash accounts and manual transactions are atomic notes, including when provider logging is set to Atomic line. A cash account stores `financeSource: manual`, `accountType: depository`, `accountSubtype: cash`, and `openingBalance`. Its dashboard balance is derived from that opening balance plus its signed cash transactions every time the model is read. No cached `current` property is stored on cash accounts. Edit `openingBalance` to correct the starting amount; edit or trash transaction notes to correct the ledger. Both transfer legs come from one note, so retrying a read, editing an amount, or deleting a transfer cannot increment an old balance twice. Existing cash/account links must remain valid.

Cash transactions store the normal finance identity, account link, date, signed amount, currency, category override, and tags. Expense amounts are negative; income amounts are positive. A transfer records `transfer-in` or `transfer-out` and `transferAccount`, and must reference another account in the same currency. Record a cash-to-cash transfer once. A transfer to/from a Plaid bank account adjusts only manual cash: the bank balance remains provider-owned. `linkedTransaction` can point to the matching imported transaction; it is a reference, not automatic matching or a provider reclassification. Transfers are excluded from spending and income totals. Existing category budgets remain USD-only.

Houses, cars, computers, and other resale assets are account notes with `financeSource: manual`, `accountType: other`, an asset subtype, `current`, and `valuationDate`. **Update value** edits the current manual estimate and date while preserving other properties and body text. Optional `purchaseTransaction` and `liabilityAccount` links connect existing notes. Values contribute once to the same-currency net worth and Resale assets total; a loan's balance is already counted separately and the link does not subtract it a second time. Updating an estimate creates no income or cash transaction. For a sale, record its actual cash receipt separately and update the asset value to zero. There are no automatic appraisals, depreciation schedules, valuation history, currency conversion, or bank writes.

All records stay in the configured finance folder and use the existing configured TPS identity key. Manual source markers prevent Plaid balances/transactions from becoming manual adjustments. Invalid amounts, dates, currencies, missing cash accounts, and invalid transfer counterpart links stop the relevant operation instead of inventing balances. Note writes propagate storage errors without updating another ledger. Metadata changes, deletion, and rename refresh the dashboard; derived balance reads do not write notes or create a feedback loop.

The dashboard header wraps in narrow panes, new actions use native buttons/menus with accessible names, and keyboard focus remains visible. Manual account cards open their source notes; assets additionally expose Update value. Source/reading/live-preview behavior remains ordinary Obsidian notes. Minimum Obsidian remains 1.12.0 and the plugin remains desktop-only; mobile can view synced notes but this release does not add a mobile runtime.

Validation: 103 automated tests cover existing Plaid/atomic behavior plus manual creation, edits, deletion, transfer conservation, bank balance ownership, stale metadata during creation, valuation links/body preservation, currency/date/path validation, and disk failure. Reloaded test-vault UI created a $100 wallet, logged a $12.50 tagged expense, created a $900 computer, and changed its value to $800. Runtime checks verified edited expenses, both transfer legs, deleted transfers, and Atomic line compatibility. Original settings and the existing 16 Sandbox accounts, 1,694 transactions, and 13 holdings were preserved. The final versioned suite, separate build, test deployment/reload, UI check, and release hashes are recorded in RELEASE-NOTES.md. Production installation is the user's BRAT pull.

# 1.1.0 — Sandbox-tested connections and summaries

Connections now include **Reconnect**, using Plaid update mode to repair authorization without creating another Item or replacing its identity map, access token, or cursor. All existing settings routes and controls remain; Reconnect sits alongside Disconnect on each connection. No new persisted settings are introduced.

Refunds and purchase credits reduce category spending. Identified credit-card repayments are transfers, so paying the card does not count the purchase twice; existing records receive the same dashboard interpretation. Account summaries separate cash, investments, and signed debt by currency. Investment account balances are authoritative, with same-currency holdings used only when the balance is unavailable. Holdings never add to an already-counted account balance. Overpaid credit/loan balances retain their asset sign. Different currencies are never added together or converted implicitly. Existing category budgets are USD-only and exclude other currencies; per-currency budgets and exchange-rate conversion are not implemented.

When Plaid identifies a posted charge as the replacement of an already-known pending charge, both provider IDs retain the same local note identity and the pending removal does not trash its tags, category, or body. Unchanged provider transaction revisions skip writes; finance metadata events during a sync no longer launch a dashboard scan for every imported note. The sync performs its final dashboard refresh. Atomic dashboards exclude unrelated finance collections while still finding moved notes linked to their account collection.

Validation uses this device's Sandbox credentials without recording them in source or reports. The actual browser Link flow connected First Platypus Bank test accounts. Regression coverage includes signed liabilities, investment valuation, currency separation, unchanged writes, collection ownership, pending-to-posted identities, and update-mode payloads. Final Sandbox results, tests, builds, and release artifact hashes are recorded in RELEASE-NOTES.md. Real institution OAuth quirks and physical mobile behavior remain separate checks; the plugin remains desktop-only. This is an account/transaction/investment dashboard with category budgets, not a double-entry accounting or tax-filing system.
...
Validated on 2026-09-12: source control/action/option/binding inventories preserved; full `npm test` passed (67 tests). Separate `npm run build` deployed to the isolated test vault. Reloaded settings routes render without generic subtitles, and route navigation leaves settings unchanged. Native navigation, focus styles, and existing narrow-screen strips are retained. Release notes record exact results, route verification, and SHA-256 hashes. Minimum Obsidian remains 1.12.0. Production installation is the user's BRAT pull. Unrelated working-tree changes are excluded.
...
### 1.1.0 final Sandbox validation — 2026-09-13

Actual Safari Link connected 14 First Platypus Bank accounts. Independently queried API data matched all 394 bank and 1,169 investment transaction amounts/dates and all account balances. The first two-year import on 1.0.0 took about 13 minutes in this iCloud test vault; 1.1.0 repeat sync took 4.7 seconds without changing record counts. A second dynamic Sandbox Item exercised pending-to-posted changes: the annotated note retained its identity, path, category, tags, and body. With both Items, repeat sync took 8.7 seconds and retained 16 accounts, 1,694 unique transactions, and 13 holdings. These are observed timings, not guarantees of initial import or real-bank latency.

Forcing `ITEM_LOGIN_REQUIRED` preserved the cursor and last-success timestamp. The actual Safari update-mode flow repaired the same Item, followed by successful sync. A budget created through the command palette/modal displayed $39.35 against a $100 limit, matching the categorized note. Final reload preserved settings and both Sandbox connections; all four settings destinations and the six dashboard metrics were checked. The desktop dashboard was visually inspected and synthetic QA results are recorded in the test vault; physical mobile and real-bank OAuth were not tested. The final version passes 91 tests and a separate production-mode build deploys only to this test vault. Public release details and hashes are in RELEASE-NOTES.md.
