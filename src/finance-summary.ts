...
export function accountSummaries(accounts: readonly FinanceAccount[], holdings: readonly FinanceHolding[]) {
  const summaries = new Map<string, {currency:string;netWorth:number;cash:number;investments:number;debt:number}>();
  for (const account of accounts) {
    const currency = account.currency || "USD";
    let summary = summaries.get(currency);
    if (!summary) summaries.set(currency, summary = {currency,netWorth:0,cash:0,investments:0,debt:0});
